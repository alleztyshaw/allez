...
export default async function handler(req, res) {
  ...
  try {
    ...
    const entities = [];
    if (client_name?.trim()) {
      entities.push({ original: client_name.trim(), token: '[CLIENT]' });
    ...
    if (advisor_name?.trim()) {
      entities.push({ original: advisor_name.trim(), token: '[ADVISOR]' });
      const parts = advisor_name.trim().split(/\s+/);
      if (parts.length >= 2) {
        entities.push({ original: parts[0], token: '[ADVISOR]' });
        entities.push({ original: parts[parts.length - 1], token: '[ADVISOR]' });
      }
    }
